return {
  ...
  config = function()
    ...
    -- https://github.com/gucheen/nvim-gc/blob/5e57b8232a14cc1735060aa6c07742711dcf3a61/lua/plugins/statusline.lua#L4

    local function get_modified_icon(bufnr)
      bufnr = bufnr or vim.api.nvim_get_current_buf()
      if vim.api.nvim_get_option_value('modified', { buf = bufnr }) then
        return '● ' -- or use '●', '•', '*', '+', '[+]' - whatever icon you prefer
      end
      return ''
    end
      ...
      text = function(_, _, width)
        if width > width_breakpoint then
          return {
            ...
            { sep.slant_right,             state.mode[2] .. 'Sep' },
          }
        end
        return {
          { ' ' .. state.mode[1]:sub(1, 1) .. ' ', state.mode[2] },
          { sep.block,                             state.mode[2] .. 'Sep' },
        }
      end,
    }
    ...
    basic.section_b = {
      ...
      text = function(bufnr, _, width)
        ...
        return { { sep.slant_right, state.mode[2] .. 'Sep' } }
      end,
    }
    ...
    b_components.file_name = function(default, modify)
      ...
      elseif modify == 'relative' then
        fnc_name = function(bufnr)
          return vim.fn.expand('%:.', false, false) -- equivalent to lualine's path = 1
        end
      end
      return function(bufnr)
    ...
    basic.section_c = {
      ...
      text = function(bufnr)
        return {
          { ' ',                                                  state.mode[2] },
          { get_modified_icon(bufnr) },
          { b_components.cache_file_name('[No Name]', 'relative') },
          { sep.block,                                            state.mode[2] .. 'Sep' },
        }
      end,
    }
    ...
    basic.section_y = {
      ...
      text = function(_, _, width)
        ...
        return { { sep.block, state.mode[2] .. 'Sep' } }
      end,
    }
